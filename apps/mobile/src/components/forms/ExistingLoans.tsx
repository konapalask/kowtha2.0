...
interface ExistingLoansFormData {
  loans: Loan[];
}

interface Props {
  initialData?: Partial<ExistingLoansFormData>;
  onSubmit: (data: ExistingLoansFormData) => void;
}
...
const ExistingLoans: React.FC<Props> = ({initialData, onSubmit}) => {
  const {
    control,
    handleSubmit,
    formState: {errors},
  } = useForm<ExistingLoansFormData>({
  ...
  const onFormSubmit = (data: ExistingLoansFormData) => {
    onSubmit(data);
  };

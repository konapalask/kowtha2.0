...
export const generateFinalReport = async (id: string, type:string) => {
  const response = await axiosInstance.get(`/loans/${id}/generate-final-report?type=${type}`, {
    responseType: 'blob',
    headers: {
...
export const getEditRequestsApi= async( status:string, loanId:string) =>{
  return await axiosInstance.get(`/edit-requests?status=${status}&loanId=${loanId}`)
}

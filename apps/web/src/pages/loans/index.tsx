...
import { Table, Button, Space, Tag, Typography, Modal, Form, Input, Upload, Drawer, Descriptions, Select, message, Card, Radio, Checkbox, Row, Col } from 'antd';
import { EditOutlined, UploadOutlined, InfoCircleOutlined, UserOutlined } from '@ant-design/icons';
...
export default function Loans() {
  ...
  const [loans, setLoans] = useState<Loan[]>([
    {
      ...
      verifications: [
        { id: 1, type: 'Permanent Address', assignmentMethod: 'Local', assignee: 'John Doe', status: 'Pending' },
        { id: 2, type: 'Work', assignmentMethod: 'Remote', office: 'Delhi', assignee: 'Jane Smith', status: 'Pending' },
      ],
    },
    {
      id: 2,
      ...
      verifications: [
        { id: 3, type: 'Current Address', assignmentMethod: 'Local', assignee: 'Jane Smith', status: 'Pending' },
      ],
    },
  ]);
  ...
  const [remoteExecutives] = useState<RemoteExecutives>({
    'Mumbai': [{ id: 3, name: 'Raj Kumar' }, { id: 4, name: 'Priya Shah' }],
    'Delhi': [{ id: 5, name: 'Amit Singh' }, { id: 6, name: 'Neha Gupta' }],
    'Bangalore': [{ id: 7, name: 'Karthik R' }, { id: 8, name: 'Divya M' }],
    'Chennai': [{ id: 9, name: 'Senthil K' }, { id: 10, name: 'Lakshmi N' }]
  });
  ...
  const handleVerificationAssign = async (
    ...
    values: { assignmentMethod: 'Local' | 'Remote'; office?: string; assignee: string }
  ) => {
    try {
      setLoading(true);
      // Mock API call
  ...
  const handleLoanInfoSave = (values: any) => {
    setLoading(true);
    setTimeout(() => {
      setLoans(loans.map(loan => loan.id === selectedLoan?.id ? { ...loan, ...values } : loan));
  ...
  const columns: ColumnsType<Loan> = [
      ...
      children: [
        {
          ...
          render: (_, record: Loan) => {
            const pav = record.verifications.find(v => v.type === 'Permanent Address');
            return pav ? <Tag color={pav.status === 'Completed' ? 'green' : 'blue'}>{pav.status}</Tag> : '-';
          },
        },
        {
          title: 'Assignee',
      ...
      children: [
        {
          ...
          key: 'cavStatus',
          render: (_, record: Loan) => {
            const cav = record.verifications.find(v => v.type === 'Current Address');
            return cav ? <Tag color={cav.status === 'Completed' ? 'green' : 'blue'}>{cav.status}</Tag> : '-';
          },
        },
        {
          title: 'Assignee',
      ...
      children: [
        {
          ...
          render: (_, record: Loan) => {
            ...
            return wv ? <Tag color={wv.status === 'Completed' ? 'green' : 'blue'}>{wv.status}</Tag> : '-';
          },
        },
        {
          title: 'Assignee',
    ...
    },
    {
      ...
      render: (status: string) => {
        const color = status === 'Pending' ? 'orange' : status === 'Approved' ? 'green' : status === 'Rejected' ? 'red' : 'blue';
  ...
  return (
    <DashboardLayout>
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <Text type="secondary">
            <InfoCircleOutlined /> The CSV file should contain the following columns:
          </Text>
          <ul>
      ...
      <Drawer
        title={
          <span>
            ...
            {selectedLoan && (
              <Tag color={
                selectedLoan.status === 'Pending' ? 'orange' :
                selectedLoan.status === 'Approved' ? 'green' :
                selectedLoan.status === 'Rejected' ? 'red' :
                selectedLoan.status === 'FieldVerificationComplete' ? 'green' :
                selectedLoan.status === 'FieldVerificationStarted' ? 'blue' :
                'default'
              } style={{ marginLeft: 8 }}>
                {(() => {
                  switch(selectedLoan.status) {
                    ...
                    case 'FieldVerificationStarted': return 'Under FV';
                    case 'FieldVerificationComplete': return 'FV Completed';
                    case 'Approved': return 'Approved';
                    case 'Rejected': return 'Rejected';
                    default: return selectedLoan.status;
                  }
                })()}
              </Tag>
            )}
          ...
          <>
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography.Title level={4} style={{ margin: 0 }}>Loan Information</Typography.Title>
                {!editLoanInfo && (
                  <Button type="link" onClick={() => setEditLoanInfo(true)} icon={<EditOutlined />}>Edit</Button>
                )}
              </div>
              {!editLoanInfo ? (
                <Descriptions 
                ...
                >
                  ...
                  <Descriptions.Item label="Uploaded At">
                    {dayjs(selectedLoan.uploadedAt).format('YYYY-MM-DD HH:mm:ss')}
                  </Descriptions.Item>
                </Descriptions>
              ) : (
                <Form
                ...
                >
                  <Row gutter={16}>
                    ...
                    <Col xs={24} sm={12}>
                      <Form.Item label="Applicant Name" name="applicantName" rules={[{ required: true, message: 'Required' }]}> <Input /> </Form.Item>
                    </Col>
                    <Col xs={24} sm={12}>
                      <Form.Item label="Address" name="applicantAddress" rules={[{ required: true, message: 'Required' }]}> <Input /> </Form.Item>
                    </Col>
                    <Col xs={24} sm={12}>
                      <Form.Item label="Loan Type" name="loanType" rules={[{ required: true, message: 'Required' }]}> <Input /> </Form.Item>
                    </Col>
                    <Col xs={24} sm={12}>
                      <Form.Item label="Bank Name" name="bankName" rules={[{ required: true, message: 'Required' }]}> <Input /> </Form.Item>
                    </Col>
                  </Row>
                  <Form.Item>
                    <Space>
            ...
            <div style={{ marginTop: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                {sameAddress ? (
                  [
                    { label: 'Permanent & Current Address', type: 'Permanent Address', merged: true },
                    { label: 'Work', type: 'Work', merged: false }
                  ].map(({ label, type, merged }) => {
                    const verification = selectedLoan.verifications.find(v =>
                      merged ? (v.type === 'Permanent Address' || v.type === 'Current Address') : v.type === type
                    );
                    return (
                      <Card
                        key={label}
                        ...
                        extra={
                          verification && (
                            <Tag color={verification.status === 'Completed' ? 'green' : 'blue'}>
                              {verification.status}
                            </Tag>
                          )
                        }
                      >
                        <Form
                          ...
                          onFinish={(values) => handleVerificationAssign(selectedLoan.id, type, values)}
                        >
                          <Form.Item
                            name="assignmentMethod"
                            label="Assignment Method"
                            rules={[{ required: true, message: 'Please select assignment method' }]}
                          >
                            <Radio.Group>
                                  ...
                                  <Select placeholder="Select field executive">
                                    {assignmentMethod === 'Local'
                                      ? fieldExecutives.map(fe => (
                                          <Option key={fe.id} value={fe.name}>{fe.name}</Option>
                                        ))
                                      : office && remoteExecutives[office]?.map((fe: FieldExecutive) => (
                                          <Option key={fe.id} value={fe.name}>{fe.name}</Option>
                                        ))
                                    }
                                  </Select>
                  ...
                  ['Permanent Address', 'Current Address', 'Work'].map((type) => {
                    const verification = selectedLoan.verifications.find(v => v.type === type);
                    return (
                      <Card
                        key={type}
                        title={type}
